...
# 业绩外支付项目
EXTRA_ACHIEVE = ['支付会员卡赠', '支付围台酒水', '支付职能招待', '支付宴请酒水', '支付会员权益'] 
# 部门列表
...
# 保存路径
writer = pd.ExcelWriter(DIR_DESKTOP + "稽核/每日现抽.xlsx")
writer_day = pd.ExcelWriter(DIR_DESKTOP + "稽核/每日业绩/" + day + "每日个人业绩.xlsx")
writer_week = pd.ExcelWriter(DIR_DESKTOP + "稽核/每周业绩/" + day + "每周个人业绩.xlsx")
writer_total_achieve = pd.ExcelWriter(DIR_DESKTOP + "稽核/业绩汇总.xlsx")
...
table["日期主单号"] = table["日期"].apply(lambda x: x.replace("-", "")) + table['主单']
table[['日期主单号', '计提成小计', '无业绩体验卡', '花单点舞小计', '计提成小计', '无业绩小计']] = table[['日期主单号', '计提成小计', '无业绩体验卡', '花单点舞小计', '计提成小计', '无业绩小计']].apply(pd.to_numeric)
...
invalid = table.query('支付职能招待 > 0')['日期主单号']
for each in invalid:
    detail.loc[detail['日期主单号'] == each, ['订房人', "部门"]] = ["散客", "散客"]

re = pd.merge(detail, plan, on="酒水项目", how='left')
re = pd.merge(re, staff, on="艺人", how="left")
re["订房现抽"] = 0
re.loc[(re['类型'] != "经理赠送") & (re['订房人'] != "自来客") & (re['订房人'] != "散客"),'订房现抽'] = re['金额']/re['单价']*re['现抽单价']
re.loc[(re['酒水项目']=="电子礼炮") & (re['类型'] == "消费"), '气氛道具扣除'] = re['金额'] * 0.25
re.to_excel(DIR_DESKTOP+'稽核/落单明细(检查用).xlsx', index=False)


# 花单汇总表
girl_award = pd.pivot_table(re.query('艺人 != ""' ), index=['日期主单号', '落单时间','日期', '艺人部门', '艺人','房台名称', '酒水类别', '支付方式'], values=['数量', '金额'], aggfunc={'数量':np.sum, '金额':np.sum}).reset_index()
total_basket = total_basket.append(girl_award).drop_duplicates(keep ='first', inplace = False).sort_values(by='日期')
total_basket.to_excel(DIR_DESKTOP + "稽核/仓库/花单汇总表.xlsx", sheet_name='汇总', index=False)

# 花单现抽
girl_award = pd.pivot_table(re.query('艺人部门=="资-Bgo有底薪" & 日期 == @day & 艺人 != ""' ), index=['日期', '艺人部门', '艺人','房台名称', '酒水类别', '支付方式'], values=['数量', '金额'], aggfunc={'数量':np.sum, '金额':np.sum}, margins=True).reset_index()
girl_award['提成金额'] = girl_award['金额']*0.5
girl_award.loc[girl_award['支付方式'] == "会员本金", '提成金额'] = girl_award['金额']*0.4
# 销售现抽
seller_award = pd.pivot_table(re.query('支付方式 != "挂账"'), index=['日期主单号','落单时间','日期', '房台名称', '酒水项目', '单价', '订房人部门', '订房人', '支付方式'], values=['数量', '订房现抽'], aggfunc={'数量':np.sum, '订房现抽':np.sum}).query('订房现抽 != 0').reset_index()

# 更新现抽汇总表, 去掉重复项, 按时间排序
total_award = total_award.append(seller_award).drop_duplicates(keep ='first', inplace = False).sort_values(by='日期')
total_award.to_excel(DIR_DESKTOP + "稽核/仓库/现抽汇总表.xlsx", sheet_name='汇总', index=False)


seller_award = seller_award[['日期', '房台名称', '酒水项目', '单价', '订房人部门', '订房人', '支付方式', '数量', '订房现抽']].query('日期 == @day').sort_values(by='单价').reset_index()
seller_award.loc['合计']= seller_award[['数量', '订房现抽']].apply(lambda x:x.sum())
del seller_award['index']
# 副卡点舞提成
second_card = pd.pivot_table(re.query('酒水项目 == "点舞(副卡专用)" & 日期 == @day'), index=['日期', '房台名称', '酒水项目'], values=['数量', '金额'], aggfunc={'数量':np.sum, '金额':np.sum}, margins=True).reset_index()
second_card['提成金额'] = second_card['金额']*0.15
...
# 电子礼炮
air = pd.pivot_table(re.query('酒水项目=="电子礼炮" & 类型=="消费"' ), index=['日期主单号', '落单时间','日期', '房台名称', '订房人部门', '订房人', '支付方式'], values=['数量', '金额'], aggfunc={'数量':np.sum, '金额':np.sum}).reset_index()
total_air = total_air.append(air).drop_duplicates(keep ='first', inplace = False).sort_values(by='日期')
total_air.to_excel(DIR_DESKTOP + "稽核/仓库/礼炮汇总表.xlsx", sheet_name='汇总', index=False)

air = pd.pivot_table(re, index='日期主单号', values='气氛道具扣除', aggfunc={"气氛道具扣除":np.sum}).reset_index().query('气氛道具扣除 != 0')
award = pd.pivot_table(re, index=['日期主单号'], values='订房现抽', aggfunc={"订房现抽":np.sum}).reset_index().query('订房现抽 != 0')


# 不同门店有所不同
main = pd.merge(table, air, on='日期主单号', how='left')
main = pd.merge(main, award, on='日期主单号', how='left').fillna(0)
...
main['周数'] = main['日期'].apply(lambda x: (datetime.datetime.strptime(x, '%Y-%m-%d').strftime('%Y-%W')))
